...
var App = function () {
	function App() {
		...
		document.querySelector('#root').innerHTML = _Elements.component;
		document.querySelector('#get_data').addEventListener('click', this.receiveData);
	}

	_createClass(App, [{
		key: 'receiveData',
		value: async function receiveData() {
			try {

				var jokeData = await _Services2.default.getJokeFromTheServer().then(function (response) {
					document.querySelector('#joke_section').innerHTML = response.data.name + ' joke: ' + response.data.joke;
				});
			} catch (error) {
...
var component = exports.component = "\n\t<header>\n\t\t<h1>Async-Await</h1>\n\t</header>\n\t<div>\n\t\t<p>Data</p>\n\t\t<input type=\"button\" value=\"Vajuta\" id=\"get_data\" />\n\t\t\n\n\t\t<div>\n\t\t\t<div id=\"joke_section\"></div>\n\t\t</div>\n\t</div>\n";
...
var Services = function () {
	...
	_createClass(Services, null, [{
		key: 'getJokeFromTheServer',
		value: function getJokeFromTheServer() {
			var received = (0, _axios2.default)({ url: 'http://localhost:3004/api/joke' });
			return received;
		}
	}]);
